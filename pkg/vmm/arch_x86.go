...
func (x86MachineBackend) postCreateVCPUs(vm *VM) error {
	...
	// hot path; all IRQ delivery now flows through makeIRQLine →
	// hv.InjectInterrupt → kvmVM.IRQLine. No per-device wiring needed
	// here; setupIRQs already published the GSI routing table.
	return nil
}
