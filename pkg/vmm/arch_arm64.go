...
func (arm64MachineBackend) setupVCPU(vm *VM, hvcpu HVVCPU, index int, kernelInfo *loader.KernelInfo) error {
	kvcpu, ok := hvcpu.(*kvmVCPU)
	if !ok {
		return fmt.Errorf("arch_arm64 setupVCPU: expected *kvmVCPU, got %T", hvcpu)
	}
	vcpu := kvcpu.vcpu
	kvmVM, err := kvmVMFromHV(vm.hvVM)
	if err != nil {
		return fmt.Errorf("arch_arm64 setupVCPU: %w", err)
	}
	// Get the preferred target type for this host.
	init, err := kvmVM.PreferredARM64Target()
	if err != nil {
...
func (arm64MachineBackend) handleExit(vm *VM, hvcpu HVVCPU) (handled bool, stop bool, err error) {
	kvcpu, ok := hvcpu.(*kvmVCPU)
	if !ok {
		return false, false, fmt.Errorf("arch_arm64 handleExit: expected *kvmVCPU, got %T", hvcpu)
	}
	vcpu := kvcpu.vcpu
...
func (arm64MachineBackend) captureVCPU(hvcpu HVVCPU) (VCPUState, error) {
	kvcpu, ok := hvcpu.(*kvmVCPU)
	if !ok {
		return VCPUState{}, fmt.Errorf("arch_arm64 captureVCPU: expected *kvmVCPU, got %T", hvcpu)
	}
	vcpu := kvcpu.vcpu
...
func (arm64MachineBackend) restoreVCPU(_ Hypervisor, hvVM HVVM, hvcpu HVVCPU, state VCPUState) error {
	if state.ARM64 == nil {
		return fmt.Errorf("restore arm64 vcpu %d: no ARM64 state in snapshot", state.ID)
	}
	kvcpu, ok := hvcpu.(*kvmVCPU)
	if !ok {
		return fmt.Errorf("arch_arm64 restoreVCPU: expected *kvmVCPU, got %T", hvcpu)
	}
	vcpu := kvcpu.vcpu
	vm, err := kvmVMFromHV(hvVM)
	if err != nil {
		return fmt.Errorf("arch_arm64 restoreVCPU: %w", err)
	}
...
func (arm64MachineBackend) restoreVMState(_ HVVM, _ *SnapshotArchState) error {
	return nil
}
